  ...
  if (data.length > 0) {
    ...
    const calleeRoute = (parent, router, i, exeUrl) => {
      ...
      const level = R.map((lv1, i2) => {
        ...
        level1.component = () => _import(level1.path);
        const btn =
        ...
        if (lv1.children) {
          if (lv1.children.every(r => r.hidden === false)) {
    ...
    asyncRouter = tree.map((lv1, i) => {
      ...
      level1.index = i;
      level1.meta.index = i;
      level1.component = () => _import(lv1.href);
      ...
      if (lv1.children) {
        ...
        if (url.length > 1) {
          ...
          level1.path = p2;
          level1.component = () => _import(url[1].split(":")[1]);
        }
      }
      const atm =
        lv1.operate && lv1.operate.match(/atm\s*:\s*(["a-zA-Z-,]+?\")/gi);
